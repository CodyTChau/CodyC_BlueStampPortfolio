...
For my final milestone, I built a robotic arm for the top of my Hexapod robot. This arm comes from the LK Cokoino Robotic Arm Kit, and comes with an arduino nano-style MCU board, a servo driver shield, three MG90S micro servos, one SG90 servo, two joystick modules, many acrylic pieces, and a variety of screws, nuts, and spacers for assembly. However, I did not have the required batteries for the given battery case. So instead, I soldered on a new battery case compatible with the AA batteries we did have in stock. Then, after assembling and programming the rest of the Cokoino arm, I began the process of integrating the arm on top of the Hexapod. By drilling holes in vacant places on both the top acrylic piece of the Hexapod and the acrylic baseplate of the Cokino arm, I used spare brass standoffs to attach the arm. However, the Hexapod was now unevenly weighted due to loading the arm more towards the front of the Hexapod to maximize the arm's range of motion. So, I decided to design and 3D print a battery case holder for both the Cokoino arm battery case and the Hexapod battery pack for the back of the Hexapod in Solidworks. Now, not only was the Hexapod balanced, but the batteries were no longer detached from the build. Additionally, I designed longer acrylic pieces because the arm was now elevated, meaning the acrylic parts initially used for the actual arm segments were too short. Finally, as for my last modification, I decided to make the Cokoino arm wirelessly controlled from my phone by installing a HM-10 bluetooth module onto the arduino nano and updating the code. Now, through an app on my iphone called LightBlue, I can send bluetooth signals to the HM-10 via letters in a textbox. It is then processed by the HM-10 and arduino nano as serial data, which tells the arduino nano to move a designated servo based on that serial UART signal. 

### Challenges

Adding the many modifications to my Hexapod proved to be a much more complicated and laborious task than I had previously imagined. To start, I experienced many difficulties with the base code of the Cokoino arm. These problems included the arm jerking up and down when moving downwards, performing two actions simultaneously with one input, and a really irritating additional feature where the Cokoino would buzz and reset to a default position after moving the right joystick right. So, I had to rewrite most of the code to smoothen out the movements of the arm, make each joystick input independent of one another, removing the additional feature, and adding new code to the left and right inputs of the right joystick to coincide with opening and closing the claw respectively. Secondly, during the integration of my two builds, one of the drill holes on the Cokoino arm baseplate was misaligned, meaning only three screws could slot into their designed places. To solve this issue, I hand filed the hole until it was aligned with the brass standoff, which added unnecessary time to the integration process. And when I received the 3D print for my battery holder, the dimensions for the screw holes were misaligned as well. So, I had to file down both the holes on the Cokoino arm baseplate and the 3D print, the edge of the base plate, the backside of the battery pack, and the surrounding sides of one of the screw holes so they would all fit. I also had to file down the battery case hole for the Cokoino arm battery case so it would slide in. And as for my final 3D print, the extended arm segments, each hole was too small by a few millimeters. So, I decided to drill into most of the holes to make them the correct diameter, and hand filed the rest. Filing down the 3D prints took an entire day each, which was unexpected, annoying, and extremely exhausting and could've been avoided had I taken more care with not only measuring the dimensions for my battery pack holder, but also giving tolerance by adjusting the dimensions slightly bigger or smaller for the 3D printer. Finally, for my last challenge, integrating the wireless arm modification into my project not only involved altering the code for the Cokoino arm, but also finding a bluetooth app on my iphone and learning how to use it with my project. However, I initially started working with the HC-05 bluetooth module, as opposed to the HM-10. Although we had this module in stock, it could only interact with an android, not my iphone. So, I had to wait to receive the HM-10 from amazon before I could fully begin working on these modifications. After receiving the HM-10, connecting it to the arduino nano proved to be much more simple than anticipated, but getting my iphone to connect with the module and send it signals was a much more complex task. But through many troubleshooting efforts, I was able to connect my iphone to the HM-10 and arduino nano and control the arm wirelessly.
